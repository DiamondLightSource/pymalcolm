...
class CAWaveformTablePart(Part):
    """Defines a float64[] `Attribute` that talks to multiple DBR_DOUBLE waveform PV,
    ...
    def __init__(self,
        ...
        self.caa = util.WaveformTableAttribute(
            TableMeta(
                ...
                writeable=False,
                elements=elements),
            util.catools.DBR_DOUBLE, pv_list, name_list, min_delta, timeout, widget, group, config,
            on_connect=self._update_display)

    def _update_display(self, connected_pv):
        if self.display_from_pv:
            el = [ind for ind in range(len(self.pv_list)) if self.pv_list[ind] == connected_pv.name]
            if len(el) == 1:
                display = Display(
