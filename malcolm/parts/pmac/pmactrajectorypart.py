...
class MotorInfo(Info):
    ...
    def _make_padded_ramp(self, v1, v2, pad_velocity, total_time):
        ...
        """
        # The time taken to ramp from v1 to pad_velocity
        t1 = self.ramp_time(v1, pad_velocity)
        # Then on to v2
        t2 = self.ramp_time(pad_velocity, v2)
    ...
    def _calculate_hat_params(self, v1, v2, acceleration, distance):
        ...
        if acceleration > 0:
            ...
            vm = -self.max_velocity
        t1 = self.ramp_time(v1, vm)
        d1 = self.ramp_distance(v1, vm, t1)
        t2 = self.ramp_time(vm, v2)
    ...
    def make_velocity_profile(self, v1, v2, distance, min_time=0.0):
        ...
        if min_time > ramp_time:
            ...
            # A ramp will cover the right distance
            it = self._make_ramp(v1, v2)
        ...
        for t, v in it:
            if v*last_v < 0:
                ...
                fraction = last_v / (last_v - v)
                time_array.append(fraction * t)
        ...
        if self.velocity_settle > 0:
            time_array.append(self.velocity_settle)
