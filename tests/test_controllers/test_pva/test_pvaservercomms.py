...
from malcolm.controllers.pva.pvaservercomms import PvaServerComms, PvaGetImplementation, PvaPutImplementation, PvaRpcImplementation, PvaEndpoint, PvaMonitorImplementation
...
#        server_mock.get_request.assert_has_calls([call("test.block", request)])
    ...
    def test_pva_get_implementation(self):
        ...
        #server.get_request = Mock(return_value="test_return_2")
        #pva.get()
        #server.get_request.assert_called_with("test.block", request)
    ...
    def test_pva_put_implementation(self):
        ...
        #server.get_request.assert_called_with("test.block", request)
        #self.assertEqual(pva.getPVStructure(), "test_return_2")
        #pva.dict_to_path = Mock()
        #pva.dict_to_value = Mock()
        #pv = Mock()
        #pva.put(pv)
