...
class PandABLutIconTest(unittest.TestCase):
    ...
    def test_lut_elements(self):
        ...
        assert get_lut_icon_elements(0xf0f0f0f) == {
            'A', 'AND', 'B', 'D', 'E', 'LUT', 'OR', 'notA', 'notB', 'notC',
            'notD', 'notE', 'edgeA', 'edgeB', 'edgeD', 'edgeE'}
        # A|B
        assert get_lut_icon_elements(0xffffff00) == {
            'AND', 'C', 'D', 'E', 'LUT', 'NOT', 'notA', 'notB', 'notC', 'notD',
            'notE', 'edgeC', 'edgeD', 'edgeE'}

    def test_symbol(self):
        # !A&!B&!C&!D
        self.o.client.get_field.return_value = "0x00000003"
        icon = SVGIcon(self.o.svg_text)
        self.o.update_icon(icon, dict(
            FUNC="~A&~B&~C&~D", TYPEA="level", TYPEB="rising",
            TYPEC="falling", TYPED="either", TYPEE="rising"))
