...
class TestSystemDetectorPVA(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for controller in \
                detector_block(mri="TESTDET", config_dir="/tmp") + \
        ...
        for controller in \
                ...
                proxy_block(mri="TESTDET", comms="PVA-CLIENT"):
...
class TestSystemMotionPVA(unittest.TestCase):
    ...
    def setUp(self):
        self.process = Process("proc")
        for controller in \
                motion_block(mri="TESTMOTION", config_dir="/tmp") + \
                pva_server_block(mri="PVA-SERVER"):
            self.process.add_controller(controller)
        self.process.start()
        self.process2 = Process("proc2")
        for controller in \
                pva_client_block(mri="PVA-CLIENT") + \
                proxy_block(mri="TESTMOTION", comms="PVA-CLIENT"):
            self.process2.add_controller(controller)
        self.process2.start()
